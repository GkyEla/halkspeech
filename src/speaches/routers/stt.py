...
def translate_file(
    ...
    with whisper_model_manager.load_model(model) as whisper:
        ...
        # BatchedInferencePipeline requires vad_filter=True or clip_timestamps
        # Force vad_filter=True when using batched mode
        batched_vad_filter = True if config.whisper.use_batched_mode else effective_vad_filter
        segments, transcription_info = whisper_model.transcribe(
            audio,
            task="translate",
            initial_prompt=prompt,
            temperature=temperature,
            vad_filter=batched_vad_filter,
        )
