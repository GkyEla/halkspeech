...
EXPOSE 8000
# Increase timeouts to support large file uploads (10-20MB audio files)
# --timeout-keep-alive: Time to keep idle connections alive (default: 5s)
# --timeout-graceful-shutdown: Time to wait for requests to complete during shutdown (default: 0s)
CMD ["uvicorn", "--factory", "speaches.main:create_app", "--timeout-keep-alive", "300", "--timeout-graceful-shutdown", "30"]
